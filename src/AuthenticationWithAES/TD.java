...
//                    }
                ...
                try{
                    ...
                    //这里需要解密
                    System.out.println("TD(AES encrypted): receive encrypted request，decrypt....");
//                        //这里开始测试
...
//                        plainText = new String(ECCModule.decrypt(test,MyPriKey));
                ...
                if(tag.equals("Request_KeyExchange")){
                    String keyStr = FromClient.getString("AESKey");
                ...
                if(tag.equals("Request_AuthenticationStart")){
                    System.out.println("TD(AES encrypted): Authentication Request received");
                    ...
                    System.out.println("TD(AES encrypted): TDH3 generated："+keyedHashTDH3);
                    //然后需要时间戳发送给客户
                    ...
                    String cipherText = aesEncryption.encrypt(JSON_time.toString(), AESKey);

                    //将JSON发给client
                    bufferedWriter.write(cipherText+"END");
                    bufferedWriter.flush();
                    System.out.println("TD(AES encrypted): ACK_TimeStamp_SentOut");

                }else if(tag.equals("ERR_finished")){
                    System.out.println("TD(AES encrypted): Err! Request for the authentication is stopped");
                    bufferedWriter.close();
                    bufferedReader.close();
                    break;
                }else if(tag.equals("DH3")){
                    System.out.println("TD(AES encrypted): Client-DH3 received");
                    //接收客户端发来的DH3的KeyedHash
                    String DH3 = FromClient.getString("DH3");
                    //判断和自己先前计算的值是否相等
                    if(DH3.equals(keyedHashTDH3)){
                        //认证成功，将ok返回给client
                        System.out.println("TD(AES encrypted): TDH-authentication pass!");
                        JSONObject JSON_result=new JSONObject();
                        JSON_result.put("tag", "ACK_OK");

                        //加密
                        String cipherTest = aesEncryption.encrypt(JSON_result.toString(), AESKey);
                        //发回客户端
