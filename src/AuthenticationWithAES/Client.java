...
//            BufferedReader sin=new BufferedReader(new InputStreamReader(System.in));
            ...
            //把公钥转换成str发送出去
            JSONKeyRequest.put("AESKey", AESkey);
            ...
            while (true){
                ...
                try{
                    ...
                    //这里需要解密
                    System.out.println("Client(AES encrypted): received the encrypted request，decrypt....");
                    plainText = aesEncryption.decrypt(receive, AESkey);
//                    System.out.println(plainText);
                ...
                if(tag.equals("ACK_KeyExchange")){
                    ...
                    String cipherTxt =aesEncryption.encrypt(JSONrequest.toString(), AESkey);
                    //将JSON发给TD,手动加入结束符号，提供判断
                    bufferedWriter.write(cipherTxt+"END");
                    bufferedWriter.flush();
                    System.out.println("Client(AES encrypted): Authentication request sent out");
                    continue;
                }
                if(tag.equals("ACK_mac&serial_timeProvided")){
                    //要接受从TD传过来的time值
                    time = authenticationFromTD.getString("time");
                    System.out.println("Client(AES encrypted): TimeStamp received "+time);
                    if(time!=null){
                        //重新计算KeyedHash
                        keyedHash = new KeyedHashGenerator().keyedHash(mac,serial,time,key);
                        System.out.println("Client(AES encrypted): DH3 generated："+keyedHash);
                        //然后需要将计算出的keyedHash结果发给TD
                        JSONObject JSON_DH3_Back=new JSONObject();
                        JSON_DH3_Back.put("tag", "DH3");
                        JSON_DH3_Back.put("DH3", keyedHash);
                        //使用TD的publicKey加密
                        String cipherTxt =aesEncryption.encrypt(JSON_DH3_Back.toString(), AESkey);

                        //将JSON发给TD
                        bufferedWriter.write(cipherTxt+"END");
                        bufferedWriter.flush();
                        System.out.println("Client(AES encrypted): DH3 sent out");
                    }else{
                        System.out.println("Client(AES encrypted): error! not a valid timestamp");
                        JSONObject ERR=new JSONObject();
                        ERR.put("tag", "ERR_finished");
                        //使用TD的publicKey加密
                        String cipherTxt =aesEncryption.encrypt(ERR.toString(), AESkey);
                        bufferedWriter.write(cipherTxt+"END");
                        bufferedWriter.flush();
                    ...
                    System.out.println("Client(AES encrypted): Err! Server refuses to respond");
                    bufferedWriter.close();
                    bufferedReader.close();
                    break;
                }else if(tag.equals("ACK_OK")){
                    System.out.println("Client(AES encrypted)：feedback received");
                    System.out.println("Client(AES encrypted): authentication pass!");
                    bufferedWriter.close();
                    bufferedReader.close();
                    break;
                }else if(tag.equals("ACK_NOT_MATCH")){
                    System.out.println("Client(AES encrypted): authentication fail!");
                    bufferedWriter.close();
                    bufferedReader.close();
                    break;
                }
